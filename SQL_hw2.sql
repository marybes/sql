...
from ((employees as e
inner join roles_employees as re
...
9. Вывести имена и должност;ь всех QA инженеров.
select employee_name, role_name
from ((employees as e
inner join roles_employees as re
on e.id = re.employee_id)
...
inner join roles as r
on r.id = re.role_id)
inner join employees_salary as es 
on e.id = es.employee_id)
...
inner join roles as r
on r.id = re.role_id)
where monthly_salary between 1770 and 2300 
order by monthly_salary; 
